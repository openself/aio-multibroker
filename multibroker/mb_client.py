...
class MBClient(ABC):
    ...
    @abstractmethod
    async def _sign_payload(self, rest_call_type: RestCallType, resource: str,
                            data: dict | None = None, params: dict | None = None,
                            headers: dict | None = None) -> None:
        pass

    @abstractmethod
    def _preprocess_rest_response(self, status_code: int, headers: 'CIMultiDictProxy[str]',
                                  body: dict | None) -> None:
        pass
    ...
    async def _create_get(self, resource: str, params: dict | None = None, headers: dict | None = None,
                          signed: bool = False, api_variable_path: str | None = None) -> dict:
        return await self._create_rest_call(RestCallType.GET, resource, None, params, headers, signed,
                                            api_variable_path)

    async def _create_post(self, resource: str, data: dict | None = None, params: dict | None = None,
                           headers: dict | None = None, signed: bool = False,
                           api_variable_path: str | None = None) -> dict:
        return await self._create_rest_call(RestCallType.POST, resource, data, params, headers, signed,
                                            api_variable_path)

    async def _create_delete(self, resource: str, data: dict | None = None, params: dict | None = None,
                             headers: dict | None = None, signed: bool = False,
                             api_variable_path: str | None = None) -> dict:
        return await self._create_rest_call(RestCallType.DELETE, resource, data, params, headers, signed,
                                            api_variable_path)

    async def _create_put(self, resource: str, data: dict | None = None, params: dict | None = None,
                          headers: dict | None = None, signed: bool = False,
                          api_variable_path: str | None = None) -> dict:
        return await self._create_rest_call(RestCallType.PUT, resource, data, params, headers, signed,
                                            api_variable_path)

    async def _create_rest_call(self, rest_call_type: RestCallType, resource: str,
                                data: dict | None = None, params: dict | None = None,
                                headers: dict | None = None, signed: bool = False,
                                api_variable_path: str | None = None) -> dict:
        with Timer('RestCall'):
            ...
            if rest_call_type == RestCallType.GET:
                rest_call = self._get_rest_session().get(resource_uri, json=data, params=params,
                                                        headers=headers, ssl=self.ssl_context)
            elif rest_call_type == RestCallType.POST:
                rest_call = self._get_rest_session().post(resource_uri, json=data, params=params,
                                                         headers=headers, ssl=self.ssl_context)
            elif rest_call_type == RestCallType.DELETE:
                rest_call = self._get_rest_session().delete(resource_uri, json=data, params=params,
                                                           headers=headers, ssl=self.ssl_context)
            elif rest_call_type == RestCallType.PUT:
                rest_call = self._get_rest_session().put(resource_uri, json=data, params=params,
                                                        headers=headers, ssl=self.ssl_context)
            else:
                raise Exception(f"Unsupported REST call type {rest_call_type}.")

            LOG.debug(f"> rest type [{rest_call_type.name}], uri [{resource_uri}], "
