...
class AlorWebsocket(WebsocketMgr):
    ...
    def __init__(
            self, subscriptions: list[Subscription], ssl_context=None,
            alor_client: AlorClient | None = None, is_demo: bool = False,
    ) -> None:
        ws_server = f'wss://api{"dev" if is_demo else ""}.alor.ru/ws'

        super().__init__(
            websocket_uri=ws_server, subscriptions=subscriptions,
            ssl_context=ssl_context, builtin_ping_interval=None,
            auto_reconnect=True, periodic_timeout_sec=5,
        )
    ...
    async def _process_message(self, websocket: Websocket, message: str) -> None:
        ...
        if http_code is not None:
            ...
            if http_code == 401:
                ...
                raise WebsocketReconnectionException(f"WS 401: {parsed.get('message', '')}")
        ...
        await self.publish_message(
            WebsocketMessage(subscription_id=guid, message=data)
        )
...
class BarsSubscription(AlorSubscription):
    ...
    def __init__(
            self,
            symbol: str = '', tf: str = '',
            seconds_from: int = 0, skip_history: bool = True,
            split_adjust: bool = False, exchange: str = '', instrument_group: str = '',
            frequency: int = 1_000_000_000, data_format: str = 'Simple',
